		...
		while (!feof($handle))
{
   ...
   $csv_array = explode(",", $handle[$i]);
   $insert_csv = array();
   //$insert_csv['ID'] = $csv_array[0];
   $insert_csv['email'] = $csv_array[0];
   $insert_csv['amount'] = $csv_array[1];
   $query = "INSERT INTO Allied(email,amount)
     VALUES('".$insert_csv['email']."','".$insert_csv['amount']."')";
...
mysql_close($connect); // closing connection
	...
	$query_order = "SELECT * FROM Allied ORDER BY Allied_ID DESC LIMIT 1";
  ...
  while ($row = mysql_fetch_array($result_order)){
	...
	print "<form enctype='multipart/form-data' action='allied.php' method='post'>";
