...
?>
                ...
                <h2><a href="#">Database</a> &raquo; <a href="#" class="active">Upload <b>Investor</b> Database</a></h2>
...
$i='1';
	while (($data = fgetcsv($handle, 10000, ",")) !== FALSE) {
		...
		{
				$import="INSERT INTO InvestorDatabase (EmailAddress) VALUES('NULL')";
//$import="INSERT INTO Email3 (EmailAddress) values ('$data[0]')";
		mysql_query($import) or die(mysql_error());
		}
		else
		{
		$import="INSERT INTO InvestorDatabase (EmailAddress, FirstName, LastName, Country, Referrer, Submitter) VALUES('".mysql_real_escape_string($data[0])."', 
               '".mysql_real_escape_string($data[1])."', 
               '".mysql_real_escape_string($data[2])."',
			   '".mysql_real_escape_string($data[3])."','$name', '$file')";
//$import="INSERT INTO Email3 (EmailAddress) values ('$data[0]')";
		mysql_query($import) or die(mysql_error());
	}	}
  ...
  while ($row = mysql_fetch_array($result_order)){
	...
	print "<form enctype='multipart/form-data' action='database.php' method='post'>";
