...
$drawing_date = $date->format('Y-m-d');
	...
	$query_amount = "SELECT * FROM ContestAmount WHERE ContestID = '2' AND Status = 'open'";
	...
	$newend = strtotime ( '+7 day -1 hour' , strtotime ( $now ) ) ;
...
echo $query_selectAllItems;
	...
	$query_insertItem = "INSERT INTO ContestAmount (DrawingStart, DrawingEnd, ContestID, Amount, Status) VALUES ('$now', '$newend', '$cid', '0.00', 'open')";
	$dberror = "";
	$ret = mysql_query($query_insertItem);
	}
  ...
  while ($row = mysql_fetch_array($result_order)){
    ...
    $subject = "There is no winner for the weekly drawing "; 
...
//We are sending this email to say no winners
   $message = '<html><head>';
    ...
    $message .= '<table>';
    $message .= '<tr><td><font color="#000000"><b>There were no winners for the weekly drawing.<br><br>Go to '.$companyurl.' to buy yours today and GOOD LUCK to all who enter!</b></td><td></td></tr>';
	...
	$to = $winner_email;
	$from = $companynoreplyemail;
    $subject = 'Congrats!!!!  You won the weekly drawing from '.$companyurl.'!!'; 
...
//We are sending this email to say there was a winner
   $message = '<html><head>';
    ...
    $message .= '<table width="100%" height="300" bgcolor="#FFFFFF"><tr><td>';
    $message .= '<table>';
	$message .= '<tr><td><font color="#000000"><b>There was a winning ticket for the weekly drawing!!  The winning ticket is '.$ticketnumber.'  The drawing amount was $'.money_format('%(#10n', $daily).'</b></td><td></td></tr>';
	...
	$newend = strtotime ( '+7 day -1 hour' , strtotime ( $now ) ) ;
	$newend = date ( 'Y-m-d H:i:s' , $newend );
    $query_insertItem = "INSERT INTO ContestAmount (DrawingStart, DrawingEnd, ContestID, Amount, Status) VALUES ('$newstart', '$newend', '$cid', '0.00', 'open')";
	$dberror = "";
	$ret = mysql_query($query_insertItem);
