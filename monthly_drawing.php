...
$drawing_date = $date->format('Y-m-d');
	...
	$newend = strtotime ( '+1 month -1 hour' , strtotime ( $now ) ) ;
...
//output each row 
  ...
  while ($row = mysql_fetch_array($result_order)){
    ...
    $subject = "There is no winner for the monthly drawing "; 
...
//We are sending this email to say no winners
   $message = '<html><head>';
    ...
    $message .= '<table>';
    $message .= '<tr><td><font color="#000000"><b>There were no winners for the monthly draw.<br><br>Go to '.$companyurl.' to buy yours today and GOOD LUCK to all who enter!</b></td><td></td></tr>';
	...
	$to = $winner_email;
	$from = $companynoreplyemail;
    $subject = 'Congrats!!!!  You won the monthly drawing from '.$companyurl.'!!'; 
...
//We are sending this email to say there was a winner
   $message = '<html><head>';
    ...
    $message .= '<table width="100%" height="300" bgcolor="#FFFFFF"><tr><td>';
    $message .= '<table>';
	$message .= '<tr><td><font color="#000000"><b>There was a winning ticket for the monthly drawing!!  The winning ticket is '.$ticketnumber.'  The drawing amount was $'.money_format('%(#10n', $daily).'</b></td><td></td></tr>';
